...
data <- read.csv("data_disagree_long_sensitivity.csv")[,2:14]
data_long <- read.csv("data_long_sensitivity.csv")[,2:14]
```
...
#Modelling primary models
##The full model with skill difference (full)
```{r}
#Get priors
get_prior(leader_behaviour ~ 0 + skill_dif * Leader_gender:Follower_gender + (1 + skill_dif:Follower_gender | Subject ), data = data, family = "bernoulli")
...
prior_check_full <- brm(leader_behaviour ~ 0 + skill_dif * Leader_gender * Follower_gender + (0 + skill_dif:Follower_gender | Subject ), prior = prior_full, data = data, sample_prior = "only",iter = 4000, family = "bernoulli", chains = 2, cores = 2)

pp_check(prior_check_full, nsamples = 100)

# Model w skill difference
m_full <- brm(
  leader_behaviour ~ 0 + skill_dif * Leader_gender * Follower_gender + (1 + skill_dif:Follower_gender | Subject),
  data = data,
  prior = prior_full,
  sample_prior=T,
...
###Plotting the model transition, full
```{r}
#Defining paths for plots
trans_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/transition/trans_plot_full.jpeg")
pp_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/pp_check/pp_plot_full.jpeg")

marginal_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/marginal/marginal_plot_full.jpeg")

#Marginal effects plot: Plotted and saved
jpeg(file=marginal_path)
print(marginal_effects(m_full))
dev.off()
...
jpeg(file=pp_path)
print(pp_check(prior_check_full, nsamples = 100))
dev.off()
...
#prior predictive check
prior_check_basic <- brm(leader_behaviour ~ 0 + Leader_gender : Follower_gender + (1 + skill_dif:Follower_gender | Subject), prior = prior_basic, data = data, sample_prior = "only",iter = 4000, family = "bernoulli", chains = 2, cores = 2)

pp_check(prior_check_basic, nsamples = 100)

#model without skill difference
m_basic <- brm(
  leader_behaviour ~ 0 + Leader_gender : Follower_gender + (1 + skill_dif:Follower_gender | Subject),
  data = data,
  prior = prior_basic,
  sample_prior=T,
  family = "bernoulli", #As we had a binary outcome, we set this to "bernoulli"
...
###Plotting the model transition, basic
```{r}
#Defining paths for plots
trans_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/transition/trans_plot_basic.jpeg")
pp_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/pp_check/pp_plot_basic.jpeg")

marginal_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/marginal/marginal_plot_basic.jpeg")

#Marginal effects plot: Plotted and saved
jpeg(file=marginal_path)
print(marginal_effects(m_basic))
dev.off()
...
#pp_check plot: Plotted and saved
jpeg(file=pp_path)
print(pp_check(prior_check_basic, nsamples = 100))
dev.off()
```
...
#prior predictive check
prior_check_mf <- brm(answer ~ dif_blue + dif_blue:Gender + (1+dif_blue|Subject), prior = prior_mf, data = data_long, sample_prior = "only",iter = 4000, family = "bernoulli", chains = 2, cores = 2)
...
##Plotting the model transition
```{r}
#Defining paths for plots
trans_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/transition/trans_plot_mf_skill.jpeg")
pp_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/pp_check/pp_plot_mf_skill.jpeg")

marginal_path = file.path("~/gender_bias_in_decision_making/Gender_bias_study_1/plots/marginal/marginal_plot_mf_skill.jpeg")

#Marginal effects plot: Plotted and saved
jpeg(file=marginal_path)
print(marginal_effects(skill_mf))
dev.off()
...
hypothesis(skill_mf, "dif_blue:GenderMale>dif_blue")

#More evidence for the alternative hypothesis i.e. women perform better than men
hypothesis(skill_mf, "dif_blue:GenderMale<dif_blue")

```


#Hypothesis testing: H1, H2 og H3
...
# H3: There is an interaction: males are more discriminative as to their followers gender than females are
hypothesis(m_basic, "(Leader_gender0:Follower_gender0 - Leader_gender0:Follower_gender1) > (Leader_gender1:Follower_gender0 - Leader_gender1:Follower_gender1)")

#There is more evidence for the contrary, i.e. females are more discriminative as to their followers gender than males are
hypothesis(m_basic, "(Leader_gender0:Follower_gender0 - Leader_gender1:Follower_gender1) < (Leader_gender1:Follower_gender0 - Leader_gender1:Follower_gender1)")
```


##H3: plotting
```{r}
H3 <- ggplot(pred, aes(x = Leader_gender, y = Estimate, fill = Follower_gender)) + 
