...
## CONFIDENCE SHOULD PROBABLY BE DELETED AS WE DO NOT INCLUDE IT IN STUDY 1
...
sensitivity_scores_roll <- read.csv("sensitivity_scores_rolling.csv")
```
...
sensitivity_scores <- select(sensitivity_scores, -c("GroupNumber", "dif_blue","answer", "joint_answer", "Correct", "Correct_joint", "dif_blue_abs"))
...
```{r}
#Subset to left data
data_left <- subset(data, select = c(GroupNumber, unique_ID_left, dif_blue, dif_blue_abs, chosen_leader, Leader_gender, Follower_gender, skill_dif, skill_dif_c, confidence_left, leader_behaviour, Sensitivity_roll_left, Gender_left, left_answer, Trial_left)) #take all data for left participant
...
```{r}
#Subset right data
data_right <- subset(data, select = c(GroupNumber, unique_ID_right, dif_blue, dif_blue_abs, chosen_leader, Leader_gender, Follower_gender, skill_dif, skill_dif_c, confidence_right, leader_behaviour, Sensitivity_roll_right, Gender_right, right_answer, Trial_right)) #Subsetting right data
...
data_long <- read.csv("data_long_sensitivity.csv")
```
